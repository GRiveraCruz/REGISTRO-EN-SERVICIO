  ...
  try {
    ...
    rows.forEach(row => {
      const worker = {
        ...
        locationId: row['Ubicacion'] || row['ubicacion'] || '',
...
app.get('/api/workers/template', (req, res) => {
  ...
  const ws = XLSX.utils.aoa_to_sheet([
    ['Nombre', 'Puesto', 'Jornada', 'Ubicacion'],
    ['Juan Pérez', 'Técnico', '07:00-17:00', ''],
    ['María López', 'Supervisor', '08:00-18:00', '']
  ]);
...
app.post('/api/locations', (req, res) => {
  ...
  const location = { id: uuidv4(), radius: 75, ...req.body, createdAt: new Date().toISOString() };
